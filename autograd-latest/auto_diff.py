...
class AddOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        # 譬如 a + b, 对a偏导为1， 对b偏导也为1， 所以直接用回传回来的额output_grade *1 就还是output_grad
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        node_to_output_grad[node] = zeroslike_op(node) # 每个节点的梯度有可能是由多条边传递过来的，这样就需要累加，所以都初始化为0

    # 和上面用的是一个数组，对于output_node（i.e. 输出的node ），自己关于自己的梯度 一定是1
    ...
    for node in reverse_topo_order:
        for inp, grad in zip(node.inputs, node.op.gradient(node, node_to_output_grad[node])):
            node_to_output_grad[inp] = node_to_output_grad[inp] + grad # 把grad累加到已有的梯度上的过程，因为可能有很多条边过来
